...
import cat.copernic.prodis.reservesprodis.databinding.FrgReservaSalaBinding
...
class ReservaSala : Fragment() {
    // Binding
    private lateinit var binding: FrgReservaSalaBinding
    ...
    // Arguments
    private lateinit var args: ReservaSalaArgs
    ...
    private val tipoReserva: String = "sala"
    ...
    override fun onCreateView(
        ...
        // Binding
        binding = FrgReservaSalaBinding.inflate(layoutInflater)
        ...
        args = ReservaSalaArgs.fromBundle(requireArguments())
        nombreReserva = args.argSalaName
        nombreReservaOriginal = args.argSala
    ...
    private fun limpiarReservas() {
        ...
        db.collection("reservas").document("salas").collection(nombreReservaOriginal)
    ...
    private fun checkHorario() {
        ...
        if (horasReservadas == 0) {
            ...
            findNavController().navigate(ReservaSalaDirections.actionReservaSalaToMisReservas())
        }
    }
    ...
    private fun checkReservasToday() {
        db.collection("reservas").document("salas").collection(nombreReservaOriginal).get()
